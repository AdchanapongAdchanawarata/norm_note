...
fn get_notes(state: State<AppState>) -> Result<Vec<NoteItem>, String> {
    ...
    for id in live_notes {
        if let Ok(Some(text)) = ws.replica().text(&id) {
            ...
            let preview = if lines.len() > 1 { lines[1].to_string() } else { "".to_string() };
...
fn save_note(id: String, content: String, state: State<AppState>) -> Result<(), String> {
    ...
    ws.replica_mut().write(&doc_id, &content).map_err(|e| e.to_string())?;
...
fn delete_note(id: String, state: State<AppState>) -> Result<(), String> {
    ...
    ws.replica_mut().delete(&doc_id).map_err(|e| e.to_string())?;
    ws.push_to_disk().map_err(|e| e.to_string())?;
...
#[tauri::command]
fn export_note_dialog(format: String, title: String, md_content: String, txt_content: String, html_content: String) -> Result<(), String> {
...
#[tauri::command]
fn import_files_dialog(folder: String, filter_type: String, state: State<AppState>) -> Result<Vec<String>, String> {
    ...
    if let Some(paths) = dialog.pick_files() {
        ...
        for path in paths {
            ...
            let is_image = filter_type == "img" || matches!(path.extension().and_then(|s| s.to_str()).unwrap_or(""), "png" | "jpg" | "jpeg" | "gif" | "webp" | "svg");
            ...
            if is_image {
                ...
                if std::fs::copy(&path, &dest).is_ok() {
                    ...
                    let content = format!("# {}\n\n![{}]({})\n", stem, stem, format!("assets://localhost/{}", asset_filename));
                    
                    let new_filename = format!("{}.md", timestamp);
                    let id = if folder.is_empty() { new_filename } else { format!("{}/{}", folder, new_filename) };
                    let doc_id = DocId::from_relative_path(std::path::Path::new(&id));
...
#[tauri::command]
fn import_image_bytes(bytes: Vec<u8>, ext: String, _state: State<AppState>) -> Result<String, String> {
    let root = get_vault_path();
    let assets_dir = root.join(".assets");
    if !assets_dir.exists() {
        std::fs::create_dir_all(&assets_dir).map_err(|e| e.to_string())?;
...
#[cfg(target_os = "macos")]
fn build_menu(app: &AppHandle, recent_notes: &[(String, String)]) -> tauri::Result<Menu<tauri::Wry>> {
    let new_note = MenuItem::with_id(app, "new-note", "New Note", true, Some("CmdOrCtrl+N"))?;
    let new_folder = MenuItem::with_id(app, "new-folder", "New Folder", true, Some("CmdOrCtrl+Shift+N"))?;
    let rename_note = MenuItem::with_id(app, "rename-note", "Rename Note...", true, None::<&str>)?;
    let duplicate_note = MenuItem::with_id(app, "duplicate-note", "Duplicate Note", true, Some("CmdOrCtrl+D"))?;
    let delete_note = MenuItem::with_id(app, "delete-note", "Delete Note", true, Some("CmdOrCtrl+Backspace"))?;
    let import_md = MenuItem::with_id(app, "import-files:md", "Markdown (.md)", true, None::<&str>)?;
    let import_img = MenuItem::with_id(app, "import-files:img", "Images (.png, .jpg)", true, None::<&str>)?;
    let import_txt = MenuItem::with_id(app, "import-files:txt", "Text (.txt, .csv)", true, None::<&str>)?;
    let import_all = MenuItem::with_id(app, "import-files:all", "All Files", true, None::<&str>)?;
    let import_submenu = Submenu::with_items(app, "Import Files...", true, &[
        &import_md,
        &import_img,
        &import_txt,
        &import_all,
    ...
    let export_html = MenuItem::with_id(app, "export-note:html", "HTML (.html)", true, None::<&str>)?;
    let export_txt = MenuItem::with_id(app, "export-note:txt", "Text (.txt)", true, None::<&str>)?;
    let export_submenu = Submenu::with_items(app, "Export Note...", true, &[
        &export_md,
        &export_pdf,
        &export_html,
        &export_txt,
    ])?;
    let backup_vault = MenuItem::with_id(app, "backup-vault", "Backup Vault...", true, None::<&str>)?;
    let restore_vault = MenuItem::with_id(app, "restore-vault", "Restore Vault...", true, None::<&str>)?;
    let settings = MenuItem::with_id(app, "settings", "Settings...", true, Some("CmdOrCtrl+,"))?;

    let app_menu = Submenu::with_items(app, "NormNote", true, &[
    ...
    if recent_notes.is_empty() {
        let no_recent = MenuItem::with_id(app, "no-recent", "No Recent Notes", false, None::<&str>)?;
        let _ = recent_submenu.append(&no_recent);
    } else {
        for (id, name) in recent_notes {
            let note_item = MenuItem::with_id(app, format!("open-recent:{}", id), name, true, None::<&str>)?;
    ...
    let toggle_sidebar = MenuItem::with_id(app, "toggle-sidebar", "Toggle Sidebar", true, Some("CmdOrCtrl+Option+S"))?;
    ...
    let actual_size = MenuItem::with_id(app, "actual-size", "Actual Size", true, Some("CmdOrCtrl+0"))?;
    let toggle_fullscreen = MenuItem::with_id(app, "toggle-fullscreen", "Toggle Full Screen", true, Some("CmdOrCtrl+Ctrl+F"))?;
    ...
    let show_notes_window = MenuItem::with_id(app, "show-notes-window", "Notes", true, Some("CmdOrCtrl+1"))?;
