...
fn test_mass_simulation() {
    ...
    let hub_ws = Arc::new(Mutex::new(Workspace::open(hub_device, VaultKey::new([42u8; 32]), &hub_root).unwrap()));
    ...
    (1..=num_personas).into_par_iter().for_each(|i| {
        ...
        // Create notes
        ws.replica_mut().write(&doc1, &format!("Hello from persona {} A", i)).unwrap();
        ws.replica_mut().write(&doc2, &format!("Hello from persona {} B", i)).unwrap();
        ws.push_to_disk().unwrap();
        
        // Edit note
        ws.replica_mut().write(&doc1, &format!("Hello from persona {} A (edited)", i)).unwrap();
        ...
        ws.replica_mut().write(&doc2, &format!("{}\n{}", text2, text1)).unwrap();
    ...
    assert_eq!(live_notes.len(), 500, "There should be exactly 500 notes remaining after merges");
    
    for note in live_notes {
        let text = hub.replica().text(&note).unwrap().unwrap();
        assert!(text.contains("(edited)"), "Text should contain the merged edit");
    }
    
    println!("Mass simulation passed successfully for {} personas!", num_personas);
}
